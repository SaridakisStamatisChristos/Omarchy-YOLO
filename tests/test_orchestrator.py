                ...
                "tasks": [
                    {
                        "id": "T1",
                        "title": "Create result",
                        "description": "Create result.txt containing autonomous.",
                        "depends_on": [],
        ...
        *,
        ...
        if "TASK T1" in prompt:
            ...
            if self.worker_calls == 1:
                return AgentResult(1, "", "synthetic failure", 0.01, ("fake",))
...
async def test_resume_gets_fresh_attempt_budget_without_erasing_attempt_numbers(
    ...
    job = db.create_job(
        ...
        base_branch=branch,
        base_commit=base,
        integration_branch="yolo/test-resume/integration",
        auto_apply=False,
    )
    fake = FailOnceWorker()
    registry = AgentRegistry(config, overrides={"fake": fake})
    orchestrator = Orchestrator(config, db, registry=registry)
    ...
    db.retry_failed_tasks(job.id)
    db.clear_stop(job.id)
    await Orchestrator(config, db, registry=registry).run_job(job.id)
    task = db.list_tasks(job.id)[0]
    assert task.attempts == 2
    assert task.state.value == "completed"
    rows = db._execute(
        "SELECT number FROM attempts WHERE task_id = ? ORDER BY number", (task.id,)
    ).fetchall()
