...
from .util import YoloError, ensure_private_dir, xdg_runtime_dir
...
async def _ensure_daemon() -> None:
    ...
    if systemctl:
        subprocess.run([systemctl, "--user", "start", "omarchy-yolo.service"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        for _ in range(8):
            try:
    ...
    with log_path.open("ab") as log:
        subprocess.Popen([sys.executable, "-m", "omarchy_yolo.cli", "daemon"], stdin=subprocess.DEVNULL, stdout=log, stderr=log, start_new_session=True, close_fds=True)
    for _ in range(20):
        try:
            await rpc_call(socket_path(), "ping")
            return
...
def _print_status(status: dict[str, Any]) -> None:
    ...
    if not job:
        ...
        return
    print(f"{job['id']}  {job['state']}  {Path(job['repo']).name}")
    print(f"goal: {job['goal']}")
    print(f"candidate: {job['integration_branch']}")
    counts = status.get("counts", {})
    if counts:
        print("tasks: " + "  ".join(f"{key}={value}" for key, value in sorted(counts.items())))
    for task in status.get("tasks", []):
        agent = task.get("preferred_agent") or "auto"
        print(f"  {task['logical_id']:<8} {task['state']:<12} attempts={task['attempts']} agent={agent}  {task['title']}")
    if job.get("final_summary"):
        print(f"summary: {job['final_summary']}")
    if job.get("error"):
        print(f"error: {job['error']}", file=sys.stderr)

async def cmd_run(args: argparse.Namespace) -> int:
    status = await _rpc("submit", {"goal": args.goal, "repo": str(Path(args.repo).resolve()), "auto_apply": args.apply})
...
async def cmd_jobs(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for job in jobs:
            print(f"{job['id']}  {job['state']:<10}  {Path(job['repo']).name:<24}  {job['goal'][:80]}")
    return 0

async def cmd_events(args: argparse.Namespace) -> int:
    events = await _rpc("events", {"job_id": args.job_id, "after_id": args.after, "limit": args.limit})
    if args.json:
        print(json.dumps(events, ensure_ascii=False, separators=(",", ":")))
    else:
        for event in events:
            payload = json.dumps(event["payload"], ensure_ascii=False)
            task = f" task={event['task_id']}" if event.get("task_id") else ""
            print(f"{event['id']:>6}  {event['kind']}{task}  {payload}")
    return 0
...
async def _watch(job_id: str) -> int:
    ...
    while True:
        ...
        for event in events:
            ...
            suffix = " " + json.dumps(payload, ensure_ascii=False) if payload else ""
            print(f"[{event['id']:05d}] {event['kind']}{suffix}", flush=True)
...
async def cmd_agents(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for name, cfg in data["configured"].items():
            ...
            print(f"{name:<12} {state:<9} {' '.join(cfg['command'])}")
    return 0
...
async def cmd_doctor(_: argparse.Namespace) -> int:
    ...
    for name, cfg_agent in cfg.agents.items():
        if not cfg_agent.enabled:
            continue
        exists = bool(cfg_agent.command and shutil.which(cfg_agent.command[0]))
        checks.append((f"agent:{name}", exists, shutil.which(cfg_agent.command[0]) or "missing"))
    for name, ok, detail in checks:
        marker = "OK" if ok else "!!"
        print(f"[{marker}] {name:<16} {detail}")
        if name in {"python", "linux", "non-root", "git"} and not ok:
            ready = False
...
def cmd_logs(args: argparse.Namespace) -> int:
    cfg = load_config()
    root = cfg.logs_dir / args.job_id
    if not root.exists():
        print(f"No logs for {args.job_id}", file=sys.stderr)
        return 1
    files = sorted(root.rglob("*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
    if args.latest and files:
        print(files[0].read_text(errors="replace")[-args.bytes:])
        return 0
    for path in files:
        print(path)
    return 0
