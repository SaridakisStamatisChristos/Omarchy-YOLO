...
11. Planner/reviewer roles require a declared read-only capability. Unknown custom worker commands are never assumed safe for review.
...
bounded utilization snapshots through status and the `runtime` RPC method.
...
CI runs Ruff, strict Mypy, branch-coverage tests, critical-module coverage floors, wheel build, and clean-environment
